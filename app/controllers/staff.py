...
from ..services.common import norm_type, _today, round2, overlap, valid_image_path, _store
...
#     #     if vtype:
#     #         vt = norm_type(vtype)
#     #         vehicles = [v for v in vehicles if norm_type(v.get("type")) == vt]
#     #
#     #     if brand:
...
#         revenue = round2(sum(float(r.get("total") or 0.0) for r in store.rentals.values()))
...
#         totals_revenue = round2(sum((float(r.get("total") or 0.0)) for r in store.rentals.values()))
#
#         # Rentals per vehicle
...
#         revenue_by_date = [{"date": k, "total": round2(v)} for k, v in sorted(rev_by_date.items())]
#
#         # Users by role
